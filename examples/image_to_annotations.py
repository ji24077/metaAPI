...
def image_to_annotations(img_fn: str, out_dir: str) -> None:
    ...
    resp = requests.post("http://15.157.188.112:8080/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    ...
    resp = requests.post("http://15.157.188.112:8080/predictions/drawn_humanoid_pose_estimator", files=data_file, verify=False)
    if resp is None or resp.status_code >= 300:
